...
void readDATA(const char* ifname="")
{
  if (ifname == "") ifname = __FILE__;
  ...
  {
    if (line.find(pattern, 0) == 0) {             // if string starts from '//DATA '
      data << line.substr(pattern.size()) <<endl;
    }
  }
}
...
time_t sec1970(const char* date)
{
  ...
  //cout<< "date = " << date << " year " << year << " mon " << mon << " day " << day << " hour " << hour << " min " << min << " sec " << sec <<endl;
...
{
  readDATA();
  cout<< data.str();    // NB: <<endl already in the stringstream 'data'

  std::time_t t = sec1970("2009.10.31 11:16:24");   // 47 - 28 = 19 characters
  cout<< "t = " << t <<endl;

  return 0;
}

void read_data(const char* ifname="")
{
  if (ifname == "") ifname = __FILE__;    // not recommended comparison
