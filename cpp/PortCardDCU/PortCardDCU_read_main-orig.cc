...
main()
{
  ...
  int nevents = 0;
  while (!file.eof())   // loop over all datasets in the input file
  {
    ...
    for (int ipc=0; ipc<32; )
    {
      // read two lines for each of 32 portcards
      std::string line;
      if (std::getline(file, line))
      {
        if (line.find("portcardName_ = ") != std::string::npos) {
          ...
          file
            ...
            >> ts_ >> eq >> ts
          ;
          ++ipc;          // portcard has been read ==> increament the number of the portcards (32 in total)
        }
        else continue;    // this is not data line, read next
      ...
      // else {
      //   //cout<< "Info: operator >> failed. file.eof() = " << file.eof() <<endl;
      //   //cout<< "End of data file: file.eof() = " << file.eof() <<endl;
      //   cout<< "End of data file " << fname.str() <<endl;
      //   cout<< "The total number of events sent is " << nevents+1 <<endl;                 // NB: nevents+1
      //   break;
      // }
    }
    // end of event
    ++nevents;
    if (debug_) cout<< "End of event (32 lines) " << nevents <<endl;
